"""Launch one development-local RunnerFact publication through production modules."""
...
async def _publish(nats_url: str, database: Path) -> dict[str, object]:
...
def main() -> int:
    ...
    print(
        json.dumps(
            asyncio.run(_publish(args.nats_url, args.database)),
            sort_keys=True,
            separators=(",", ":"),
        )
    )
