...
def test_on_entry_filled_exchange_submits_sl_and_tp():
    ...
    s._sltp_coordinator.submit_stop_loss.assert_called_once_with(ctx, signal, quantity=None)
    s._sltp_coordinator.submit_take_profit.assert_called_once_with(ctx, signal, quantity=None)
...
def test_on_entry_filled_hybrid_safety_sl_plus_tick():
    ...
    s._sltp_coordinator.submit_safety_stop_loss.assert_called_once_with(ctx, signal, quantity=None)
    s._sltp_coordinator.submit_stop_loss.assert_not_called()
    s._sltp_coordinator.submit_native_trailing.assert_not_called()
    ctx.tick_monitor.init_position.assert_called_once_with(
        entry_price=Decimal("100"), is_long=True, entry_atr=Decimal("2"), quantity=Decimal("1")
