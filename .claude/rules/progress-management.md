...
Plan 的 `Files` 段只列源码、把测试留给各 Task 的「写失败测试」一句话，会让文件清单审计
无法机械核对测试面——审计能查出「改了计划外的源文件」，查不出「测试根本没写」。

每个 Fix / Task 的 `Files` 同时列出它要新增或修改的测试文件。新建的测试 harness、被改到的
既有测试文件都算，后者还牵动 close-out 的重新计数（见下一节）。
...
**动了别人数过的测试文件，就得在自己的 close-out 里重数一遍。** 核对的是**最新**认领某文件
的那份 close-out —— 早先的行是历史，不会因为后来有人往同一个文件加测试就变成错的。所以
plan N 只要碰了 plan M 数过的文件，这张表里就必须有那一行；探针会点名是哪份 close-out 对不
上。改写更早的数字不是选项：那是用一个滚动值替换掉"那个 plan 交付了什么"的记录。
