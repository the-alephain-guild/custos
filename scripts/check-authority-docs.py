...
def verify_runtime_candidate_promotion(manifest: dict[str, Any], errors: list[str]) -> None:
    ...
    if publication.get("source") != {
        ...
        "revision": "4afffb96b1a768fb34f66692d4bb7f96652aeccf",
    }:
...
def verify_artifact_runtime(manifest: dict[str, Any], errors: list[str]) -> None:
    ...
    expected_publication = {
        ...
        "release_id": "v1-team-runtime-multiarch-20260810-4afffb96b1a7",
        "owner_receipt_sha256": "b7fca7c14deba4ad3b0566684a2bdad02fb2374102190dfb0e2fe4095ce51194",
        "provenance_sha256": "9c9d14ad8cbda098e06f80e49816181f0a8ea8ad58f6e0ad7cc3bfde92c93c2b",
        "sbom_sha256": "7885a81589326e5c8ddabffe9479c385f956d3faaf2e71d535f82d4753dceecd",
        "sigstore_bundle_sha256": "ae5c3c5b8d30c99a218e2ea115f8a5e176d14ac462b9ebd858828b65a6b7811f",
        "signature_verification_sha256": "add665df37d3f168f1516aae1a491272f637df577e15b34d04094171b405814c",
...
def verify_runner_nats_transport(manifest: dict[str, Any], errors: list[str]) -> None:
    ...
    if not isinstance(truth, dict):
        ...
        for key in (
            ...
            "same_event_crucible_runner_fact_projection_observed",
            ...
            "same_event_daemon_restart_completed",
            "same_event_encrypted_vaults_reloaded_after_restart",
            "same_event_transport_rotation_completed",
            "same_event_rotation_pending_operation_cleared",
        ...
        if truth.get("same_event_daemon_launch_count") != 3:
            errors.append("runner NATS transport daemon launch count differs")
        if truth.get("same_event_transport_credential_generation") != 2:
            errors.append("runner NATS transport credential generation differs")
        if truth.get("same_event_runner_fact_projection_work_count") != 1:
        ...
        if truth.get("joint_full_daemon_gate") != {
            ...
            "command_ack_after_lifecycle_commit": True,
            "runner_fact_puback_observed": True,
            "production_runner_fact_consumer_launched": True,
            "crucible_runner_fact_projection_observed": True,
            "crucible_runner_fact_projection_work_count": 1,
            "canonical_runner_fact_stream": "CRUCIBLE_RUNNER_FACT_V1",
            "encrypted_machine_vault_used": True,
            "encrypted_transport_vault_used": True,
            "plaintext_machine_bootstrap_removed": True,
            "daemon_launch_count": 3,
            "daemon_restart_completed": True,
            "encrypted_vaults_reloaded_after_restart": True,
            "transport_credential_generation": 2,
    ...
    if not isinstance(snapshot, dict):
        ...
        or snapshot.get("daemon_launch_count_in_gate") != 3
        or snapshot.get("daemon_restart_completed_in_gate") is not True
        or snapshot.get("encrypted_vaults_reloaded_after_restart_in_gate") is not True
        or snapshot.get("transport_credential_generation_in_gate") != 2
        or snapshot.get("transport_rotation_completed_in_gate") is not True
        or snapshot.get("rotation_pending_operation_cleared_in_gate") is not True
        or snapshot.get("rotation_revocation_receipt_verified_in_gate") is not True
        or snapshot.get("old_generation_reconnect_denied_in_gate") is not True
        or snapshot.get("rotated_vault_reloaded_by_daemon_in_gate") is not True
...
def verify_runner_fact_authority(errors: list[str]) -> None:
    ...
    if state.get("status") != "LOCAL_PRODUCTION_SERVICE_PHASE_B_ACCEPTED_RUNTIME_OPEN":
    ...
    transport_receipt_path = resolve(
        "docs/authority/receipts/custos-runner-nats-transport-v1-consumer-receipt.json"
    )
    transport_receipt_payload = (
        transport_receipt_path.read_bytes() if transport_receipt_path.is_file() else b""
    )
    if state.get("authenticated_transport_receipt") != {
        "path": "docs/authority/receipts/custos-runner-nats-transport-v1-consumer-receipt.json",
        "status": "LOCAL_CRUCIBLE_PRODUCTION_SERVICES_PINNED_JOINT_RUNTIME_OPEN",
        "launched_publisher_process": True,
        "signed_command_verified": True,
        "sandbox_engine_ready": True,
        "command_ack_after_lifecycle_commit": True,
        "jetstream_puback_observed": True,
        ...
        "size_bytes": len(transport_receipt_payload),
        "deployed_credential": False,
    }:
        errors.append("ecosystem RunnerFact V1 authenticated transport receipt differs")
