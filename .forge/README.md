...
| [19](plans/2026-07/19-crucible-command-runner-fact-runtime-convergence.md) | Crucible command + single RunnerFact state store + production runtime convergence | ⏳ In progress (T2-T8a and same-event local production flow ready; T9-T10 open) | Custos `a449b22` and Crucible `6a15952` prove one pinned-CA TLS event from isolated authority issuance through encrypted-vault restart/rotation, policy/command, ACK-after-lifecycle, RunnerFact PubAck and same-batch PostgreSQL projection. | Materialize the immutable PS StrategyRelease in this full daemon flow -> lock runtime RC -> PS56 exact-candidate acceptance | Exact runtime BOM/RC, immutable artifact activation, deployed services, Phase B close-out, live and production readiness remain false. |
