...
| [19](plans/2026-07/19-crucible-command-runner-fact-runtime-convergence.md) | Crucible command + single RunnerFact state store + production runtime convergence | ⏳ In progress (T2-T8a and local production-service evidence ready; T9-T10 open) | Custos `ba562a9` binds the preprovisioned control durable and Crucible `79b5acb` projects the same lifecycle batch. Crucible `2a851b2` proves CR99 owner policy; `b6e3399` receipts CR100 signer/provisioner/server on pinned-CA TLS and proves readback-verified JWT/durable command ACK. | Add immutable PS artifact materialization and same-event joint Custos policy/transport receipt -> locked runtime RC -> Plan 90B -> PS56 exact-candidate acceptance | Local Custos daemon and Crucible production-service gates pass independently. Exact runtime BOM/RC, immutable artifact activation, same-event joint deployed services, Phase B close-out, live and production readiness remain false. |
