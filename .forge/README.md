...
| [19](plans/2026-07/19-crucible-command-runner-fact-runtime-convergence.md) | Crucible command + single RunnerFact state store + production runtime convergence | ⏳ In progress (T2-T8a and same-event local transport consumer ready; T9-T10 open) | Custos `ba562a9` binds the preprovisioned control durable and Crucible `79b5acb` projects the same lifecycle batch. Crucible `2a851b2` proves CR99 owner policy; `be97225` launches CR100 signer/provisioner/server and real Custos `1b2f72d` transport consumption on the same pinned-CA TLS event. | Combine service-issued transport, CR99 policy and immutable PS material in the full Custos daemon -> locked runtime RC -> Plan 90B -> PS56 exact-candidate acceptance | Exact runtime BOM/RC, immutable artifact activation, full-daemon same-event composition, deployed services, Phase B close-out, live and production readiness remain false. |
