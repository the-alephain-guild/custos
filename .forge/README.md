...
| [01](plans/2026-09/01-nautilus-trader-2-0-upgrade.md) | NautilusTrader 1.230.0 → fork 2.0.0rc5 升级 | ⏳ In Progress (2026-09-12) | **19 行进度表中 14 行 ✅，不得 close-out**（未完：1a-2 / 1b / 2a / 2b / 14）：Task 1b 的三条判据只满足一条——fork 最新 tag `v2.0.0rc4` 内 `crates/adapters/sodex` 文件数为 0，本 plan 钉的 `3fe857a351` 无 tag 可达，亦无 Release wheel；Task 1a-2（Docker `nt-builder`）未做；Task 2a 须与 2b 同批；Task 2b ❌ Blocked on PS producer BOM + Crucible consumer receipt 重签（`engine_version` 跨仓字段）。阶段性报告与红线 gate 满足度表见 plan 文件 | SoDEX sandbox / testnet 通道（live 本 plan 不交付）；PS 策略在 2.0 上运行 | 把 toolkit（adapter 13,705 行）、daemon engine host（`src/custos/engines/nautilus/` 2,091 行）与 PS 策略本体（8 文件）升到公会 fork 2.0.0rc5，以直接消费 Rust SoDEX 适配器。**multi_session_scope: true**，6 Slice / 16 Task（含 Task 0 与 2a/2b）/ 跨 2 仓。审查 `b0ff52a`（2 C / 4 H）+ 修复 `68eab22` 后的定案：D1 两阶段：1a fork 活跃期用 uv git 源钉 sha，Docker 链把 nautilus-trader 移出 hash 清单、`nt-builder` 阶段在同一 sha 上 `maturin build` 并对账；1b fork 打 tag 后切带 sha256 的 wheel，未完成不得 close-out；不用 path 源（会击穿 3.11 base、`verify-base-clean` 与 Docker runtime lock）；D2 撤回，解释器保持 3.12（cp313 `.so` 是未跟踪的本机构建产物，非约束）；D3 PS `shared/nautilus` 维持 Plan 60 Slice E 的 defer 不动；D4 白名单改为按 mode 的三个集合，SoDEX 只进 sandbox / testnet，live 集合不变；D5b `StrategyConfig` 根类形态起草期定稿。基线：Task 0 先把 4 个既有 mypy 错误归零，typecheck 验收改为全绿。两条待 owner 确认的假设见偏离日志（D2 撤回、不交付 SoDEX live）。 |
