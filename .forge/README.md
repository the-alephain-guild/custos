...
| [26](plans/2026-07/26-attachment-state-outlives-the-engine.md) | a restart keeps the previous process's attachment state: the reconciler reloads container_id along with generation, so it believes it is attached to something it never created | 🔲 Not started | none — reproducible on current code | the offline lane's first deployment after any restart; philosophers-stone works around it today | One cause, two symptoms, both hit on 2026-07-30. A higher generation after a restart is dispatched to reconfigure because container_id is non-empty, and the engine refuses it as a structural reconfigure although nothing structural changed — only the attachment was lost. The same generation after a restart takes the equality branch and reports healthy without engaging the engine at all, so a consumer's wait-status passes while nothing runs. The reconciler cannot check: the OfflineEngine protocol has no attachment query, so it can only trust the field it persisted. Generation is right to survive a restart; attachment is not. Owner chose option B on 2026-07-30: add that attachment query to OfflineEngine — deliberately not to the runtime_checkable ExecutionEngineProtocol, whose presence-based isinstance assertions would silently change meaning for every implementer. |
