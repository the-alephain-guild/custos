...
| [01](plans/2026-09/01-nautilus-trader-2-0-upgrade.md) | NautilusTrader 1.230.0 → fork 2.0.0rc5 升级 | ⏳ In Progress (2026-09-14) | **19 行进度表中 15 行 ✅，不得 close-out**：Task 1b已发布`guild-v2.0.0rc5+sodex.1`三平台wheel并将Custos/PS切到hash-pinned URL；剩余2a须与2b同批，2b仍需PS producer BOM + Crucible consumer receipt重签（`engine_version`跨仓字段），Task14据此保持⏳。 | SoDEX sandbox / testnet 通道（live 本 plan 不交付）；PS 策略在 2.0 上运行 | 把 toolkit（adapter 13,705 行）、daemon engine host（`src/custos/engines/nautilus/` 2,091 行）与 PS 策略本体（8 文件）升到公会 fork 2.0.0rc5，以直接消费 Rust SoDEX 适配器。**multi_session_scope: true**，6 Slice / 16 Task（含 Task 0 与 2a/2b）/ 跨 2 仓。审查 `b0ff52a`（2 C / 4 H）+ 修复 `68eab22` 后的定案：D1已完成从git source过渡到三平台发布wheel；D2撤回、解释器保持3.12；D3 PS `shared/nautilus`维持defer；D4 SoDEX仅进sandbox/testnet；D5b `StrategyConfig`形态已落地。 |
