...
| [19](plans/2026-07/19-crucible-command-runner-fact-runtime-convergence.md) | Crucible command + single RunnerFact state store + production runtime convergence | ⏳ In progress (T2-T8a and clone-local control-durable-to-engine-to-authenticated-PubAck ready; T9-T10 open) | Custos pins Crucible command/material `5778397`, production authority `10b85e4`, NATS authority `e2499bb` and RunnerFact receipt `818242c`; `fe70bc4` binds the preprovisioned control durable, consumes the signed command, reaches sandbox engine readiness, commits before broker ACK and publishes the lifecycle fact under replacement TLS/User-JWT authority. The separate same-batch development gate reaches Crucible `506afcd` projection. | Combine immutable PS artifact materialization + authenticated Custos engine + Crucible PG projection -> locked runtime RC -> Plan 90B -> PS56 exact-candidate acceptance | Local revocation, control-durable command execution, authenticated command-derived publication and cross-repo projection each pass. One unified owner round trip, exact runtime BOM/RC, Phase B, live and production readiness remain false. |
