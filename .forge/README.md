...
| [01](plans/2026-09/01-nautilus-trader-2-0-upgrade.md) | NautilusTrader 1.230.0 → fork 2.0.0rc5 升级 | 🔲 Not started (2026-09-11) | fork 先发布 cp312 wheel（Task 1 Step 0）；Task 2b ❌ Blocked on PS producer BOM + Crucible consumer receipt 重签（`engine_version` 跨仓字段） | SoDEX sandbox / testnet 通道（live 本 plan 不交付）；PS 策略在 2.0 上运行 | 把 toolkit（adapter 13,705 行）、daemon engine host（`src/custos/engines/nautilus/` 2,091 行）与 PS 策略本体（8 文件）升到公会 fork 2.0.0rc5，以直接消费 Rust SoDEX 适配器。**multi_session_scope: true**，6 Slice / 16 Task（含 Task 0 与 2a/2b）/ 跨 2 仓。审查 `b0ff52a`（2 C / 4 H）+ 修复 `68eab22` 后的定案：D1 fork 发布 wheel、两仓以带 sha256 的 wheel 引用消费，不用 path 源（path 源会击穿 3.11 base、`verify-base-clean` 与 Docker runtime lock）；D2 撤回，解释器保持 3.12（cp313 `.so` 是未跟踪的本机构建产物，非约束）；D3 PS `shared/nautilus` 维持 Plan 60 Slice E 的 defer 不动；D4 白名单改为按 mode 的三个集合，SoDEX 只进 sandbox / testnet，live 集合不变；D5b `StrategyConfig` 根类形态起草期定稿。基线：Task 0 先把 4 个既有 mypy 错误归零，typecheck 验收改为全绿。两条待 owner 确认的假设见偏离日志（D2 撤回、不交付 SoDEX live）。 |
