...
| [19](plans/2026-07/19-crucible-command-runner-fact-runtime-convergence.md) | Crucible command + single RunnerFact state store + production runtime convergence | ⏳ In progress (T2-T8a, development runtime and production authority consumer ready; T9-T10 open) | Custos pins Crucible command/material `5778397`, production authority `10b85e4` and NATS authority `e2499bb`, retains one exact-event command fingerprint, consumes both mutually exclusive material branches directly and keeps one RunnerFact SQLite/outbox module. | Immutable OCI execution-byte materialization + daemon trust composition -> real PG/NATS/engine command-to-fact receipt -> corrected Plan 90 Phase A -> locked runtime RC -> Plan 90B -> PS56 exact-candidate acceptance | Production authority consumer passes 30 focused tests plus mypy/Ruff; launched SIM/LIVE, immutable materialization, exact runtime BOM/RC, Phase B, live and production readiness remain false. |
