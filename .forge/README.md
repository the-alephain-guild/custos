...
| [01](plans/2026-09/01-nautilus-trader-2-0-upgrade.md) | NautilusTrader 1.230.0 → fork 2.0.0rc5 升级 | 🔲 Not started (2026-09-11) | 无（Slice A 为本 plan 内部前置） | SoDEX live 通道；PS 策略在 2.0 上运行 | 把 toolkit（adapter 13,705 行）、daemon engine host（`src/custos/engines/nautilus/` 2,091 行）与 PS 策略本体（8 文件）升到公会 fork 2.0.0rc5，以直接消费 Rust SoDEX 适配器。**multi_session_scope: true**，6 Slice / 14 Task / 跨 2 仓。三条 owner 定案：D1 fork 走 uv path 依赖 + 解释器升 3.13（已知代价：单仓 clone 不可装，wheel/OCI 化另起 follow-up）；D3 PS `shared/nautilus` 维持 Plan 60 Slice E 的 defer 不动（与 2.0 正交）；D4 G6 venue 白名单只加 SoDEX 两个 connector，维持「白名单 == 已接线 connector」的等号契约，不改为 NT 支持的全部交易所。基线须先钉住：`make toolkit-typecheck` 在干净主干上已有 4 个既有 mypy 错误，验收判据是「不增加、不掩盖」而非全绿。 |
