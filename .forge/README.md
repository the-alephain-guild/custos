...
| [19](plans/2026-07/19-crucible-command-runner-fact-runtime-convergence.md) | Crucible command + single RunnerFact state store + production runtime convergence | ⏳ In progress (T2-T8a and clone-local authenticated command-to-Crucible-PG round trip ready; T9-T10 open) | Custos `ba562a9` binds the preprovisioned control durable and Crucible `79b5acb` projects the same lifecycle batch. Custos `ad7728e` now atomically records broker stream/sequence PubAck evidence before retiring the outbox; `aeca1fe` proves it against real JetStream. | Add immutable PS artifact materialization and production-issued policy/transport receipts -> locked runtime RC -> Plan 90B -> PS56 exact-candidate acceptance | The unified clone-local runtime round trip and durable PubAck checkpoint pass. Exact runtime BOM/RC, real daemon/services, immutable artifact activation, Phase B close-out, live and production readiness remain false. |
