...
| [01](plans/2026-09/01-nautilus-trader-2-0-upgrade.md) | NautilusTrader 1.230.0 → fork 2.0.0rc5 升级 | ✅ Completed (2026-09-14) | 无。三平台 engine wheel、Custos toolkit RC7、PS producer BOM、Crucible/PS consumer handoff 全部闭合；最终 Custos `make verify` 2472 passed / 25 skipped / 1 xfailed，toolkit mypy 41 + 60 files strict zero；PS 998 passed / 18 skipped。 | SoDEX sandbox / testnet 通道（live 本 plan 不交付）；PS 策略在 2.0 上运行 | 把 toolkit、daemon engine host 与 PS 策略本体升到公会 fork `2.0.0rc5+sodex.1`。RC7 真实 OCI 发布 run `34836525964`、独立 promotion run `34836829889`；manifest `sha256:ecedc35e…d2dd8`。内部源码/合同以 Git+CI 管理，历史收据保留其原修订语义。 |
...
| [18](plans/2026-07/18-typed-toolkit-strategy-contracts.md) | typed toolkit + strategy execution ABI + verified artifact consumer | ⏳ In progress (RC5, PS unique-V1 RC2, Crucible acceptance, clean-head local full stack and attested Custos candidate ready) | Custos run `31361924833` published source `b25a5f5` at exact digest `sha256:d66c2534...`; owner receipt `aa0cd736...`. | coordinated final-owner digest lock -> unchanged promotion -> deployed acceptance; no Speculum gate | Candidate publication is complete. Runtime, live and production readiness remain false pending downstream owner receipts. |
| [19](plans/2026-07/19-crucible-command-runner-fact-runtime-convergence.md) | Crucible command + single RunnerFact state store + production runtime convergence | ⏳ In progress (T9 local evidence and current runtime candidate accepted; cross-repo lock and T10 open) | Clean-head full loop passed under closeout `2f60d4c3...`; current Custos image is `ghcr.io/the-alephain-guild/custos@sha256:d66c2534...`. | coordinated image lock -> deployed migration/policy/transport acceptance -> PS56 exact-candidate acceptance | Local demo and candidate image are accepted. Deployed Phase-B close-out, live and production readiness remain false. |
| [20](plans/2026-07/20-custos-docs-site-scaffold.md) | custos.alephain.com 文档站 (Docusaurus + i18n + versioning) | ⏳ In progress (6/12 task; T1-T5 + T9 ✅, T6 翻译 4/46 章, T7/T8/T10/T11/T12 未开) | 无 hard-dep (消费 Plans 14-17 已落地的 `docs/**`) | T10 DNS handoff (CEO 手工 CNAME) | Sessions 1-3 落地 (`9f194a5`..`1c594fd`); gh-pages 已部署 `a60bd8b` (2026-07-21, 96 页)。**CRITICAL 已处置** (DEVIATION-07, 实测复核 2026-08-03): disclosure gate 已建并前置于 CI 构建之前 (`docs-site/scripts/check-disclosure.mjs`, 自测 35/35, 全站 92 文件 0 命中); 三章整章重写; 纪律写进 `docs-site/README.md`。**已发布的 gh-pages 也已覆盖** —— 泄漏快照 `a60bd8b` 之后又部署了 `b7e1bee` 与 `d27a153`，扫当前发布树: Philosophers-Stone / Speculum / Synedrion / Athanor / search_path / arx_live / ecosystem-authority 各 0 命中。站上仍有的 `crucible` 字样全部是 gate 显式豁免的 **wire 契约面** (NATS subject `crucible.runner_fact.*` · 签名域 `CRUCIBLE-RUNNER-FACT-BATCH-V1\0` · CLI flag `--crucible-domain-*`) —— 操作者必须逐字输入、消费方必须逐字订阅，改它属协议迁移而非文档清理，登记在本文件 §Deferred 2。**剩余的是 T6-T8 / T10-T12，不是泄漏。** |
...
| [32](plans/2026-08/32-the-producer-moved-and-only-the-consumer-has-a-gate.md) | the producer moved the runner-command golden and only the consumer has a gate — which CI never runs | ✅ Completed (2026-08-08) — one-way producer receipt handoff and independent-clone gate verified | none — reproducible on current code | workspace-checkout `make verify` (`check-authority` is permanently red); and the missing evidence that Custos accepts the producer's *current* bytes | Found while closing plan 30. Not whitespace: crucible-rust `2a9e1b9` (2026-08-03) added `cooldown_seconds: 300` inside `risk_policy` and moved `risk_policy_digest` with it. Reverting the producer to its previous revision gives bytes **identical** to Custos's current copy, so `2a9e1b9` is the single source of drift — one day old, not rot. **Not a runtime rejection**: Custos treats `risk_policy` as opaque (`deployment.py:344` — "without interpreting them"), recomputing the digest over the body, and the producer moved both together, so a real command would verify. What is broken is narrower and worse-shaped: Custos's golden, its `.sha256` sidecar and both authority indexes agree with each other on bytes the producer no longer emits — self-consistent and wrong (C7's shape) — so every test consuming that golden proves acceptance of a shape that no longer exists. And the gating is backwards: Custos has two gates but both hang off `optional_sibling_path`, so they only fire where both repos are checked out and never in CI, while crucible-rust's own authority script has no sibling comparison at all (`grep sibling` → 0 hits). The party that can break it has no gate; the party with the gate cannot see it |
...
| [2026-09/25 — the gate must cover the API that exists](fixes/2026-09/25-the-gate-must-cover-the-api-that-exists.md) | ✅ Completed (2026-09-21) | RR-1/2/3（fix03-recheck 报告首次被处理）：批量改单绕过执行门、金额计算给原生接口传 Decimal、改单撤单按旧签名调用；覆盖面改为从原生 API 枚举 |
| [2026-09/26 — containment must contain, and startup is not a breach](fixes/2026-09/26-containment-must-contain-and-startup-is-not-a-breach.md) | ✅ Completed (2026-09-21) | RR-4 + RR-8：启动期空余额不再被读成运行中失联而永久冻结（复用离线通道验证过的 readiness 等待）；熔断会先撤掉增险挂单再平仓。**RR-8 的「确认与重试」显式未做**，见该 plan 偏离日志 |
| [2026-09/27 — a policy renewal must reach the live boundary](fixes/2026-09/27-a-policy-renewal-must-reach-the-live-boundary.md) | ✅ Completed (2026-09-21) | RR-5：策略换版推进了数据库 head，却不碰活着的 boundary，于是换版之后每一单都被拒到下次 deploy 为止。boundary 现在可以就地采纳新策略并保住冻结 / 高水位 / 旧修订下的敞口；消费者在落盘成功后通知，通知抛错大声但不 nak |
| [2026-09/28 — two ledgers and a database that cannot account for itself](fixes/2026-09/28-two-ledgers-and-a-database-that-cannot-account-for-itself.md) | ✅ Completed (2026-09-21) | RR-6/7/9（fix03-recheck 报告最后三项，至此 9 项全闭环）：OKX 把同一笔手续费在 realized PnL 与 commission 行里说了两遍，改为报毛值；旧库缺 position 归属被静默接受、直到第一次真实平仓才冻结，改为启动即拒；Binance 成交行的 fee 贴的是计价币而非实际扣费币。**RR-6 的投影侧跨 venue 回归属 Crucible，显式移交**，见该 plan 偏离日志 |
| [2026-09/29 — containment must look before it claims](fixes/2026-09/29-containment-must-look-before-it-claims.md) | ✅ Completed (2026-09-21) | RR-8 验收剩下的「有确认、有重试」——fix 26 曾声明这需要另起一个工作面，动手前核实那条理由已不成立：确认回路早在关停路径上跑着（`_flatten_and_confirm_shutdown`）。熔断现在复用同一形状：轮询 venue、重发仍在的撤单与平仓、按期限收尾；超时只记不抛（抛出会掀掉整条监督任务）。至此 RR-8 五个验收分句全部有测试对应 |
| [2026-09/30 — answers outliving their question](fixes/2026-09/30-answers-outliving-their-question.md) | ✅ Completed (2026-09-21) | fix-recheck 报告七项 P1：改单按**改之后**那张委托判定（冻结时把平多 1 改成卖 2 曾开出空头）、熔断撤增险挂单不再挂在「有持仓」之后、策略换版只到本模式的边界、半仓被止损后余量成交要重新初始化新仓保护、readiness 每轮重问（重启不再继承上一个节点的判定）、落盘失败的冻结会重试、回撤基准按 bar 采样而非按入场决策。教训记为 C32 |
