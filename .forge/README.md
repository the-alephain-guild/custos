...
| [18](plans/2026-07/18-typed-toolkit-strategy-contracts.md) | typed toolkit + strategy execution ABI + verified artifact consumer | ⏳ In progress (business-named RC5 READY; sandbox development-artifact/runtime 10/10; T5e/T7-T9 blocked on real PS/Crucible receipts) | Current toolkit code, schemas, Custos↔Crucible handoff, RC5 authority and local sandbox runtime are focused verified; PS must now publish the immutable team artifact for Crucible acceptance | RC5 ✅ -> PS54 -> CR88 C6 -> T5d-A -> CR89 -> T5d-B/Plan19 T2 -> T5e -> Plan19 T5; no Speculum gate | Custos owns execution ABI and runtime verification. Earlier RC bytes remain registry audit evidence only; local sandbox success does not enable testnet/live or replace authenticated StrategyRelease resolution. |
| [19](plans/2026-07/19-crucible-command-runner-fact-runtime-convergence.md) | Crucible command + single RunnerFact state store + production runtime convergence | ⏳ In progress (T2-T8a code scopes ready; T7C/T15 sole-V1 NATS consumer clone-local verified; T8b Phase-A reopened; T9-T10 open) | Custos now consumes Crucible `e2499bb` through one begin+poll V1, persists the pending operation and local User seed before network I/O, resumes the same operation after restart, and requires broker receipts for rotate/revoke; all prior activate/challenge/evidence routes and DTOs are deleted. | CR99 publication/consumption -> launched CR100 signer/provisioner/server + Custos round trip -> corrected Plan 90 Phase A -> locked runtime RC -> Plan 90B -> PS56 exact-candidate acceptance | Focused NATS consumer gates pass, but launched owner-policy consumption, end-to-end SIM/LIVE runtime receipt, corrected Phase A, exact runtime BOM/RC, Phase B, live and production readiness remain false. |
