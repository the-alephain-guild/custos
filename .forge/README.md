...
| [19](plans/2026-07/19-crucible-command-runner-fact-runtime-convergence.md) | Crucible command + single RunnerFact state store + production runtime convergence | ⏳ In progress (T2-T8a, immutable artifact composition and development-local real JetStream PubAck ready; T9-T10 open) | Custos pins Crucible command/material `5778397`, production authority `10b85e4`, NATS authority `e2499bb` and RunnerFact consumer receipt `a36b63a`; Custos `c7d6f8b` launches an independent process using the production outbox/signer/publisher, receives a real PubAck and clears SQLite afterward. | Launched Crucible HTTP/PG + registry/Sigstore + authenticated NATS/engine same-batch receipt -> locked runtime RC -> Plan 90B -> PS56 exact-candidate acceptance | Local NATS revocation and RunnerFact publication each pass `1/1`; Phase-A cross-language and real-container consumer acceptance pass. Launched daemon/engine/Crucible round trip, exact runtime BOM/RC, Phase B, live and production readiness remain false. |
