...
| [19](plans/2026-07/19-crucible-command-runner-fact-runtime-convergence.md) | Crucible command + single RunnerFact state store + production runtime convergence | ⏳ In progress (T2-T8a, immutable artifact composition and development-local same-batch RunnerFact handoff ready; T9-T10 open) | Custos pins Crucible command/material `5778397`, production authority `10b85e4`, NATS authority `e2499bb` and RunnerFact receipt `818242c`; Custos `c7d6f8b` launches an independent publisher, receives PubAck, and the same dynamic batch reaches Crucible `506afcd` accepted state plus lifecycle projector work. | Launched Crucible HTTP/PG + registry/Sigstore + authenticated NATS/daemon/engine receipt -> locked runtime RC -> Plan 90B -> PS56 exact-candidate acceptance | Local NATS revocation, publication and cross-repo same-batch gates pass; Phase-A exact-byte consumer acceptance passes. Launched authenticated daemon/engine round trip, exact runtime BOM/RC, Phase B, live and production readiness remain false. |
