...
| [18](plans/2026-07/18-typed-toolkit-strategy-contracts.md) | typed toolkit + strategy execution ABI + verified artifact consumer | ⏳ In progress (RC5 READY; PS unique-V1 RC2 and Custos runtime image published; local full-stack accepted) | PS run `31291672169` published exact OCI/Sigstore bytes; local ARX `09342f4` + Crucible `dda4e77` + Custos `56cd64a` acceptance passed. Crucible's PS-artifact acceptance and unchanged final promotion remain open. | RC5 ✅ -> PS54 publication ✅ -> CR88 acceptance -> final unchanged promotion -> deployed acceptance; no Speculum gate | Custos owns execution ABI and runtime verification. Published bytes and local receipt do not enable testnet/live without downstream owner receipts. |
| [19](plans/2026-07/19-crucible-command-runner-fact-runtime-convergence.md) | Crucible command + single RunnerFact state store + production runtime convergence | ⏳ In progress (attested image and local full stack ready; T10 open) | Custos run `31352721692` published signed image digest `sha256:6bf1f551...`; the current three-repo harness passed PostgreSQL/NATS, command/engine, 14 RunnerFacts, projections and Chromium under close-out `ecbc393d...`. | Crucible accept exact PS artifact -> coordinated eight-image lock -> deployed migration/transport acceptance -> PS56 exact-candidate acceptance | Local demo is accepted. Deployed Phase-B close-out, live and production readiness remain false. |
