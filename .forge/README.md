...
| [19](plans/2026-07/19-crucible-command-runner-fact-runtime-convergence.md) | Crucible command + single RunnerFact state store + production runtime convergence | ⏳ In progress (T2-T8a and immutable production artifact composition ready; T9-T10 open) | Custos pins Crucible command/material `5778397`, production authority `10b85e4` and NATS authority `e2499bb`; `d926ab0` adds immutable OCI materialization and all-or-none daemon trust composition while retaining one RunnerFact SQLite/outbox module. | Real Crucible HTTP/PG + registry/Sigstore + NATS/engine command-to-fact receipt -> corrected Plan 90 Phase A -> locked runtime RC -> Plan 90B -> PS56 exact-candidate acceptance | `make verify-base-clean` passes 581 tests plus authority, zero-rewrite and both strict toolkit typing gates; launched SIM/LIVE, exact runtime BOM/RC, Phase B, live and production readiness remain false. |
