...
| [18](plans/2026-07/18-typed-toolkit-strategy-contracts.md) | typed toolkit + strategy execution ABI + verified artifact consumer | ⏳ In progress (RC5, PS unique-V1 RC2, Crucible acceptance and clean-head local full stack ready) | ARX `0d8c92b` + Crucible `f18d290` + Custos `a83e6f6` accepted the native StrategyRelease through settlement and Chromium; closeout `2f60d4c3...`. | publish current Custos image -> final unchanged promotion -> deployed acceptance; no Speculum gate | Custos owns execution ABI and runtime verification. Local evidence does not enable testnet/live without current-image and downstream owner receipts. |
| [19](plans/2026-07/19-crucible-command-runner-fact-runtime-convergence.md) | Crucible command + single RunnerFact state store + production runtime convergence | ⏳ In progress (T9 local evidence accepted; current image and T10 open) | Clean-head full loop passed PostgreSQL/NATS, production-daemon artifact activation, two command streams, 14 RunnerFacts, projections, two settlements and Chromium under closeout `2f60d4c3...`. | publish/attest current Custos head -> coordinated image lock -> deployed migration/transport acceptance -> PS56 exact-candidate acceptance | Local demo is accepted. Deployed Phase-B close-out, live and production readiness remain false. |
