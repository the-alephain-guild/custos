...
| [19](plans/2026-07/19-crucible-command-runner-fact-runtime-convergence.md) | Crucible command + single RunnerFact state store + production runtime convergence | ⏳ In progress (T2-T8a and clone-local authenticated command-to-Crucible-PG round trip ready; T9-T10 open) | Custos `ba562a9` binds the preprovisioned control durable and Crucible `79b5acb` projects the same lifecycle batch. Custos `ad7728e` atomically records broker PubAck evidence; Crucible `2a851b2` now adds real production-server CR99 owner/policy/readiness evidence. | Add immutable PS artifact materialization and isolated CR100 signer/provisioner plus joint deployed policy/transport receipts -> locked runtime RC -> Plan 90B -> PS56 exact-candidate acceptance | The unified clone-local runtime round trip, durable PubAck and local Crucible production-server checkpoints pass. Exact runtime BOM/RC, immutable artifact activation, joint deployed services, Phase B close-out, live and production readiness remain false. |
