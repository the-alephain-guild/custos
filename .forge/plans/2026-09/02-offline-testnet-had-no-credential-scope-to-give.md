...
**那个 pending 是惰性的，不是坏的**：不主动重算，被触发就算得出来，算完自己就清了。

第一版修法猜错了触发点：遍历持仓各调一次 `unrealized_pnl`。它让判词从「没有价格」变成了
一句更具体的抱怨，但仍然起不来。读 fork 源码才看清契约 ——
`crates/portfolio/src/portfolio.rs:1122` 的文档写着这个记录由 `mark_values` 维护、
「能重新估值后就移除」，而 `calculate_unrealized_pnl`（同文件 2249 行）根本不碰它。
真正执行估值并更新记录的是 `equity`（775 行的文档明说这一点，实现里
`update_missing_price_state` 被调用两次）。

也就是说：**`missing_price_instruments` 报告的是上一次估值的发现，它自己不估值**。custos
先读它、后调 `equity`，读到的永远是上一轮的结论 —— 而第一轮就发生在启动时，持仓刚由对账
交过来、价格还没到。

所以修法是调换顺序：先 `equity`，再读判词。一行位置的事。

第一版之所以看起来「有效」（探针里 missing 确实变空了），是因为那个探针在调
`unrealized_pnl` 之前先读了 `equity` 做诊断输出 —— 清状态的是 `equity`，不是我以为的那个
调用。**探针自己改变了它要观测的东西**，而我把结果算在了另一件事头上。

没有放松任何拒绝：估值跑完仍缺的照样 fail-closed。两处变异全红：把读判词挪回算权益之前
（复现出问题的顺序）、判词恒空。

替身也照真实契约改了：原先它把清状态的能力挂在 `unrealized_pnl` 上，那是按我的错误理解
造的 —— 替身照着猜测造，就只能证实猜测。
