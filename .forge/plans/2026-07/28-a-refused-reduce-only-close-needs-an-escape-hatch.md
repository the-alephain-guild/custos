...
**推荐 C 的变体，不推荐 A。** 起 plan 时我先倾向 A，想清楚 `-2022` 的含义后改了：

`-2022 ReduceOnly Order is rejected` 有**两种**互斥的意思 —— ① demo 引擎坏了（lesson #14：真有仓、
零挂单，仍被拒）；② **确实已经没有仓可减了**（本地视图过期）。②才是这个错误码的正常含义。选项 A 想用
「NT 仍报该仓 open」来区分二者，但那正是可能过期的那份视图 —— **用存疑的证据去判断证据是否存疑**，
分不开。而误判成①时下的普通反向单，会开出一个新仓。

不需要走 B（把凭据搬进策略层）。NautilusTrader 自己就能问交易所：`LiveExecutionClient` 有
`generate_position_status_reports`（镜像内实测存在），对账时用的就是它，凭据已经在那一层。所以：
**先通过 NT 向交易所查一次真实持仓，交易所确认有仓才允许下普通单。** 判定逻辑仍放
`strategy_core.py`（owner 指定的落点），只是它依据的事实来自交易所而不是本地记录。

实施前必须先验证一件事：那个查询从策略侧**能不能干净地拿到**（它在 execution client 上，不在
`Strategy` 上）。若拿不到，则**不要把 fallback 做成自动的** —— 改为把门禁判断结果大声记进日志、留人工
介入，比自动开一张可能反向的单安全。
