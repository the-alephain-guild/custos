...
> **`6f0c0e1` + `cbbff0a` 已真机复验**（2026-08-03，见 §真机复验）：46 小时连跑里 demo 引擎坏了 11 次，
> 逃生口 10 次全部在下一根 bar 内平掉；武装依据双向成立（19 次平仓中 9 次照常 reduce-only、10 次转普通单、
> 0 次无依据转）。**仍未验**：一次性上限未被触发、flatten 路径未运行、`emergency_close` 在本通道仍不可达
> **Created**: 2026-07-30
...
### 真机复验 `6f0c0e1` + `cbbff0a`（2026-08-01 → 08-03，连跑 46 小时）

上面那次是 `9f38691`，**在两个补丁之前**。这次是带着它们跑的：进容器核验过
`plan_close_attempt` / `CloseAttempt` / `close_all_positions_with_fallback` /
`_close_position_with_fallback` 都在，且 `signal_execution.py:267-280` 就是 `cbbff0a`
那段（`attempt is not CloseAttempt.PLAIN` → `mark_plain_close_submitted()`）。

46 小时里 demo 引擎坏了 **11 次**，逃生口走了 **10 次**，每次都在下一根 bar 内收敛：

```
06:17:00  Reduce-only order rejected      -2022
06:18:00  Closing without reduce_only
06:18:01  Position CLOSED  realized_pnl=+0.237
```

十次全是这个形状（`10:20/10:21`、`10:45/10:46`、`12:23/12:24`、`01:08/01:09`、`04:01/04:02`、
`06:01/06:02`、`06:50/06:51`、`06:57/06:58`、`03:52/03:53`），没有一次落空。

**武装依据（`6f0c0e1` 的前半）成立**，而且是双向的证据：19 次平仓里 **9 次照常用
`reduce_only`**、**10 次转普通单**，且**没有一次是在未记录到拒绝的情况下转普通单**。它该关的时候
关着，该开的时候开 —— 只看"开了 10 次"证不到这一点，要连"另外 9 次没开"一起看。

#### 仍未验的部分（不要把上面读成整份 plan 通过）

| 项 | 状态 | 为什么没验到 |
|---|---|---|
| 一次性上限（`6f0c0e1` 的后半）| **未触发** | 十次episode 每次一发就平掉了，守卫没被要求拒绝第二次；日志里 `already been submitted` 零命中 |
| flatten 路径（`strategy_core.py:489`）| **未触发** | 本轮熔断没跳闸，flatten 从未运行 |
| `emergency_close` | **仍不可达** | 与原文相同，离线通道不收命令 |

三条接入路径这轮只走通了一条 —— 就是 plan 自己说的「本通道唯一当前会真正触发的」那条。

## 未做（不要按"已完成"读本 plan）

1. **一次性上限、flatten 路径仍未真机触发。** 2026-08-03 那轮复验确认了武装依据与常规出场路径
   （§真机复验），但十次 episode 每次一发即平，守卫从未被要求拒绝第二次；flatten 也因熔断未跳闸而
   没运行。同一条纪律反过来也成立：**不得因为"这轮验到了逃生口"就宣布整份 plan 通过**。
