...
### owner 选定 B1-b，已落地（`e077341`，2026-08-01）

`OfflineExposureGuard` 在评估某个 deployment 前先问引擎是否就绪，未就绪就跳过 —— **等待不是跳闸**，
被跳过的 deployment 原封不动，breaker 仍允许新 generation。

**判据是同一份计算。** 新增 `deployment_ready(instance_id)`（非阻塞轮询），与 `wait_ready` 共用抽出的
`_readiness_checks()`。刻意不让守卫自己判：那会变成对同一个事实的第二意见，两边迟早漂移。

**两个不能变成的东西，各有测试钉住：**

| 风险 | 处理 |
|---|---|
| **永远瞎** —— 引擎卡在半启动就永不被守卫 | 等待**有界**（`READINESS_TIMEOUT_SECS=120`），超时后照常评估 → 落到 unreliable → fail closed。**就是原来的行为，只是推迟了**，这是正确的失败方向 |
| **静默跳过** —— 老引擎没有这个查询就悄悄不守 | 答不出就立即评估（与原行为一致），但**记一条** `offline_exposure_readiness_unknown`，且只记一次。一个悄悄没在跑的安全检查比一个吵闹地没在跑的更糟 |

120 秒不是拍的：NT 自报 `reconciliation_startup_delay_secs=10`，真机实测 4.9 秒完成，取一个数量级以上的余量 ——
**等久一点的代价是一个还没开始交易的 deployment 短暂无守；放弃太早的代价正是这次要消除的那次误跳闸。**

sandbox host 也实现了该查询（进程内模拟，deploy 完即就绪）—— 不实现的话整条 sandbox 通道都会被记成
"引擎答不出就绪"。

`_Watched` 保持 frozen，启动进度另放一个可变的 `_Startup` 字段：**身份是固定的，进度不是**。
没有用第二个字典，因为两个按 spec_id 建的集合就是两次在 `release` 里忘掉其中一个的机会。

新增 14 条测试；全仓 2275 passed；`ruff` 干净；authority gate 通过。
...
1. ~~**B1（有界宽限窗口）**~~ **owner 选 B1-b，已落地（`e077341`）** —— 等就绪而非等时间；
   必要性先被我降级、又被真机时间线证伪并回升，全过程见上。**尚未真机复验。**
2. **真机复验** —— A / B2 两支 / C 的订阅均已真机确认（见上表与第二跑）；**尚未复验的是**
   `69e153e`（mark price 类型）与 `e077341`（B1-b 就绪门）。
...
- ~~**testnet 上有一个遗留空仓，且交易所侧状态在逐轮累积。**~~ **已不成立（2026-08-03 实测）。**
  交易所侧现在是干净的：重启对账报 `Initialized 0 open orders`，当前那笔 `-0.0071` 是 `05:19` 策略
  自己开的新仓，不是这里说的那个 `-0.0070`。2026-08-01→08-03 连跑 46 小时期间 19 次开平全部干净收尾，
  止损单收支平（20 提交 / 19 撤销确认 / 0 成交），**没有再累积**；那张挂在 `65275.10` 的 stop 在该窗口
  日志里也已无踪迹。**无法确定它具体是哪一刻、被哪条路径清掉的**（更早的日志已不存在），能确定的只是
  它现在不在了 —— 所以下面这段保留作历史，不要照它去处理一个已经不存在的状态。原文：

  - **原记录：** `-0.0070 BTC`（`BTCUSDT-PERP`，均价
