...
| REFLECT-04-1 | 自省 | supervised restart 原先重放 applied outcome，不能更新 replacement handle；改为 durable degraded → recovered ready | ✅ |
| REFLECT-04-2 | 自省 | applied 覆盖保护原先误伤合法 post-apply terminal；收窄到 command-intake 终态入口 | ✅ |
...
| `tests/test_runner_fact_store.py` | 14 |
