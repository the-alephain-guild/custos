...
Custos 是一个跑在你自己机器上的守护进程。它接收签名指令，用**从不离开这台机器**的凭据对接交易场所执行，并以签名声明的形式回报实际发生了什么。
...
**ARX** 认证你是谁、授权你所请求的事、持有部署记录、决定什么应该在跑。它从不持有场所凭据，也从不下单。
...
场所凭据以 `sops`+`age` 加密文件形式存放在 `~/.arx/vault/` 下，一把密钥一个文件。解密在构造场所客户端的那一刻于进程内完成；明文从不写入状态、日志、指令或事实。
...
spec 标识是配置来源的凭证 —— 它记录的是**配置了什么**，而不是**哪个正在运行的东西**做了某件事。同一份不可变 spec 的两个实例是两个独立的东西，一次作用到错误实例上的重试是一次真实事故。
