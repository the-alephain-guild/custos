...
6. 在不可变激活根下校验并激活精确产物。精确重投会重新载入持久激活，绝不导入可变源码路径。
7. 本地解析签名凭据 scope，经引擎生命周期 supervisor 应用，并把已校验产物作为必需输入传入。
...
其中三个是传输层自己的确认动作，而它们之间的区别正是整个设计的要害：
...
`deployment_instance_id` 为键。凭据按签名凭据 scope 索引，每次使用都绑定到精确实例。事实保留 `deployment_spec_id`，仅用于记录跑的是哪份不可变配置。
